...
main()
{
  ...
  /*cout<<"Input number of Rows & Columns"<<endl;
  cin>>userDimensionInput;*/
  ...
  mpi_result = MPI_Iscatter(RowDominantMatrix.data(), rows_per_process * nCols, MPI_DOUBLE,local_A, rows_per_process * nCols, MPI_DOUBLE, 0,MPI_COMM_WORLD, &request_A);
  check_mpi_error(mpi_result);
  ...
  for(int i = 0; i < rows_per_process; i++)
    {
      ...
      cout <<"\nProcessor "<< mpi_rank+1 <<   " compute Row " << i+1 <<" with a local summed value of: "<< local_y[i] << std::endl;
    }
  ...
  mpi_result =MPI_Igather(local_y, rows_per_process, MPI_DOUBLE, y, rows_per_process,MPI_DOUBLE, 0, MPI_COMM_WORLD, &request_y);
  check_mpi_error(mpi_result);
  ...
  /*if(mpi_rank == 0)
    {
      cout << ColumnDominantMatrix << endl;
    }

  mpi_result =MPI_Ibcast(vec.data(), mpi_size, MPI_DOUBLE, 0, MPI_COMM_WORLD, &request_x);
  check_mpi_error(mpi_result);

  // cout << "fine line 136" << endl;
  //  Step 2: Non-blocking scatter rows of matrix A
  mpi_result = MPI_Iscatter(ColumnDominantMatrix.data(), rows_per_process * nCols, MPI_DOUBLE,
                            local_A, rows_per_process * nCols, MPI_DOUBLE, 0,
                            MPI_COMM_WORLD, &request_A);
  check_mpi_error(mpi_result);
  ...
  elapsed_time = end_time - start_time;
  std::cout << "Process " << mpi_rank << " computation time for ColumnDominant: " << elapsed_time
            << " seconds" << std::endl;
*/
